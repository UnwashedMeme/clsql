...
(defmethod database-create-sequence (sequence-name
                                     (database generic-postgresql-database))
  (let ((cmd (join "CREATE SEQUENCE " (escaped-database-identifier sequence-name database))))
  (database-execute-command cmd database)))

(defmethod database-drop-sequence (sequence-name
                                   (database generic-postgresql-database))
  (database-execute-command
   (join "DROP SEQUENCE " (escaped-database-identifier sequence-name database))
   database))
...
(defmethod database-sequence-next (sequence-name
  ...
  (values
   (%get-int
    (caar
     (database-query
      (join "SELECT NEXTVAL ('" (escaped-database-identifier sequence-name) "')")
      database nil nil)))))
...
(defmethod database-sequence-last (sequence-name (database generic-postgresql-database))
  (values
   (%get-int
    (caar
     (database-query
      (join "SELECT LAST_VALUE FROM " (escaped-database-identifier sequence-name))
      database nil nil)))))
...
(defun binding-to-param (binding)
  (typecase binding
    (string
     (join "'" (sql-escape-quotes binding) "'"))
    (t
     binding)))
